...
SCHEMA_SQL: Final = (
    ...
    "status TEXT NOT NULL CHECK (status IN ('pending','running','retry_wait','success','no_results_captured','failed','cancelled','skipped_existing')), "
    "attempts INTEGER NOT NULL DEFAULT 0 CHECK (attempts >= 0), "
    "max_attempts INTEGER NOT NULL DEFAULT 2 CHECK (max_attempts >= 1), "
    "retryable INTEGER NOT NULL DEFAULT 0 CHECK (retryable IN (0,1)), "
    "created_at TEXT NOT NULL, updated_at TEXT NOT NULL, started_at TEXT, finished_at TEXT, "
    "next_attempt_at TEXT, captured_at TEXT, page_state TEXT NOT NULL DEFAULT '', search_url TEXT NOT NULL DEFAULT '', "
    ...
    "event_type TEXT NOT NULL, message TEXT NOT NULL, data_json TEXT NOT NULL DEFAULT '{}', created_at TEXT NOT NULL, "
    "FOREIGN KEY (run_id) REFERENCES runs(id) ON DELETE CASCADE, FOREIGN KEY (job_id) REFERENCES jobs(id) ON DELETE CASCADE); "
    ...
    "CREATE INDEX IF NOT EXISTS idx_jobs_retry_due ON jobs(run_id, status, next_attempt_at); "
    "CREATE INDEX IF NOT EXISTS idx_events_run_created ON events(run_id, created_at, id);"
)
